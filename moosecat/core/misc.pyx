...
def register_external_logs(Client client):
    ...
    if client is not None:
        c.moose_misc_catch_external_logs(client._p())
    else:
...
def is_valid_query_tag(tag):
    b_tag = bytify(tag)
    return c.moose_store_qp_is_valid_tag(b_tag, len(b_tag))


def query_abbrev_to_full(abbrev):
    b_abbrev = bytify(abbrev) + b':'
    full = stringify(<char *>c.moose_store_qp_tag_abbrev_to_full(b_abbrev, len(b_abbrev)))
    return full[:-1]
...
cdef char * parse_query_bytes(b_query) except *:
    ...
    result = c.moose_store_qp_parse(b_query, &warning, &warning_pos)
...
cdef void * wrap_ThreadCallback(c.MooseMetadataThreads * _, object data, object user_data) with gil:
    try:
...
cdef void * wrap_DeliverCallback(c.MooseMetadataThreads * _, object data, object user_data) with gil:
    try:
...
cdef class MetadataThreads:
    cdef c.MooseMetadataThreads * _threads
    ...
    def __init__(self, thread_func, deliver_func, max_threads=15):
        ...
        self._threads = c.moose_mdthreads_new(
    ...
    def push(self, data):
        Py_INCREF(data)
        c.moose_mdthreads_push(self._threads, <void *>data)

    def forward(self, result):
        Py_INCREF(result)
        c.moose_mdthreads_forward(self._threads, <void*>result)

    def __dealloc__(self):
        if self._threads is not NULL:
            self._data_ref = None
            c.moose_mdthreads_free(self._threads)
...
cdef void * wrap_ZeroconfCallback(c.MooseZeroconfBrowser * browser, object data) with gil:
    try:
        data[0](data[1])
    except:
...
cdef class ZeroconfBrowser:
    ...
    '''
    cdef c.MooseZeroconfBrowser * _browser

...
G_DEFINE_TYPE_WITH_PRIVATE(MooseStore, moose_store, G_TYPE_OBJECT);
 ...
 */
int MooseJobPrios[] =
    {[MOOSE_OPER_DESERIALIZE] = -1,     [MOOSE_OPER_LISTALLINFO] = -1,
     [MOOSE_OPER_PLCHANGES] = +0,       [MOOSE_OPER_SPL_LOAD] = +1,
     [MOOSE_OPER_SPL_UPDATE] = +1,      [MOOSE_OPER_UPDATE_META] = +1,
     [MOOSE_OPER_DB_SEARCH] = +2,       [MOOSE_OPER_DIR_SEARCH] = +2,
     [MOOSE_OPER_SPL_QUERY] = +2,       [MOOSE_OPER_WRITE_DATABASE] = +3,
     [MOOSE_OPER_FIND_SONG_BY_ID] = +4, [MOOSE_OPER_UNDEFINED] = 10};

/**
 * Map MooseOpFinishedEnum members to meaningful strings
 */
const char *MooseJobNames[] = {[MOOSE_OPER_DESERIALIZE] = "DESERIALIZE",
 ...
 * @return a job id.
 */
static long moose_store_send_job_no_args(MooseStore *self, MooseStoreOperation op) {
...
static char *moose_store_construct_full_dbpath(MooseStore *self, const char *directory) {
    ...
    char *path = g_strdup_printf("%s%cmoosecat_%s:%d.sqlite",
                                 (directory) ? directory : ".",
                                 G_DIR_SEPARATOR,
                                 priv->mirrored_host,
                                 priv->mirrored_port);
...
static int moose_store_check_if_db_is_still_valid(MooseStore *self, const char *db_path) {
    ...
    if(g_file_test(db_path, G_FILE_TEST_IS_REGULAR) == TRUE) {
        ...
        if((exist_check = moose_gunzip(zip_path)) == false) {
            ...
            moose_message("database: Unzipping %s done (took %2.3fs).", zip_path,
                          g_timer_elapsed(zip_timer, NULL));
    ...
    if(sqlite3_open(db_path, &self->priv->handle) != SQLITE_OK) {
        moose_warning("database: %s cannot be opened (corrupted?), creating new.",
                      db_path);
        goto close_handle;
    }
    ...
    if(cached_port != moose_client_get_port(self->priv->client)) {
        moose_warning("database: %s's port changed (old=%d, new=%d), creating new.",
    ...
    if(g_strcmp0(cached_hostname, moose_client_get_host(self->priv->client)) != 0) {
        moose_warning("database: %s's host changed (old=%s, new=%s), creating new.",
                      db_path, cached_hostname,
                      moose_client_get_host(self->priv->client));
        goto close_handle;
    }
...
static void moose_store_buildup(MooseStore *self) {
    ...
    if((song_count = moose_store_check_if_db_is_still_valid(self, db_path)) < 0) {
        ...
        self->priv->stack =
            moose_playlist_new_full(100, (GDestroyNotify)moose_song_unref);
        ...
        self->priv->stack =
            moose_playlist_new_full(song_count + 1, (GDestroyNotify)moose_song_unref);
 ...
 *       #7 Since the data is fresher than what might be on disk, we'll write it there
 *later.
 *
 ...
 */
static void moose_store_update_callback(MooseClient *client,
                                        MooseIdle events,
                                        MooseStore *self) {
    if((events & (MOOSE_IDLE_DATABASE | MOOSE_IDLE_QUEUE | MOOSE_IDLE_STORED_PLAYLIST)) ==
       0) {
 ...
 */
static void moose_store_connectivity_callback(MooseClient *client,
                                              bool server_changed,
                                              MooseStore *self) {
    g_assert(self && client && self->priv->client == client);
...
void *moose_store_job_execute_callback(
    ...
    moose_stprv_lock(self->priv);
    {
        ...
        if(data->op & MOOSE_OPER_DESERIALIZE) {
            ...
            data->op |=
                (MOOSE_OPER_PLCHANGES | MOOSE_OPER_SPL_UPDATE | MOOSE_OPER_UPDATE_META);
        ...
        if(data->op & MOOSE_OPER_DB_SEARCH) {
            moose_stprv_select_to_stack(self->priv, data->match_clause, data->queue_only,
                                        data->out_stack, data->length_limit);
            result = data->out_stack;
        }

        if(data->op & MOOSE_OPER_DIR_SEARCH) {
            moose_stprv_query_directories(self->priv, data->out_stack,
                                          data->dir_directory, data->dir_depth);
            result = data->out_stack;
        }

        if(data->op & MOOSE_OPER_SPL_QUERY) {
            moose_stprv_spl_select_playlist(self->priv, data->out_stack,
        ...
        if(data->op & MOOSE_OPER_WRITE_DATABASE) {
            if(self->priv->write_to_disk) {
                char *full_path =
                    moose_store_construct_full_dbpath(self, self->priv->db_directory);
        ...
        if(data->op & MOOSE_OPER_FIND_SONG_BY_ID) {
            data->out_stack =
                moose_store_find_song_by_id_impl(self, data->needle_song_id);
            result = data->out_stack;
        }
        ...
        if(data->op &
           (0 | MOOSE_OPER_LISTALLINFO | MOOSE_OPER_PLCHANGES | MOOSE_OPER_SPL_UPDATE |
            MOOSE_OPER_SPL_LOAD | MOOSE_OPER_UPDATE_META)) {
...
long moose_store_playlist_load(MooseStore *self, const char *playlist_name) {
    ...
    return moose_job_manager_send(self->priv->jm, MooseJobPrios[MOOSE_OPER_SPL_LOAD],
                                  data);
}
...
long moose_store_playlist_query(MooseStore *self, MoosePlaylist *stack,
    ...
    return moose_job_manager_send(self->priv->jm, MooseJobPrios[MOOSE_OPER_SPL_QUERY],
                                  data);
}
...
long moose_store_query_directories(MooseStore *self, MoosePlaylist *stack,
    ...
    return moose_job_manager_send(self->priv->jm, MooseJobPrios[MOOSE_OPER_DIR_SEARCH],
                                  data);
}
...
long moose_store_playlist_get_all_known(MooseStore *self, MoosePlaylist *stack) {
    ...
    return moose_job_manager_send(self->priv->jm, MooseJobPrios[MOOSE_OPER_SPL_LIST_ALL],
                                  data);
}
...
long moose_store_query(MooseStore *self, const char *match_clause, gboolean queue_only,
    ...
    return moose_job_manager_send(self->priv->jm, MooseJobPrios[MOOSE_OPER_DB_SEARCH],
                                  data);
}
...
static GList *moose_store_get_playlists_impl(MooseStore *self,
                                             int (*func)(MooseStorePrivate *,
                                                         GPtrArray *)) {
    g_assert(self);
    ...
    moose_stprv_lock(self->priv);
    {
        ...
        for(unsigned i = 0; i < stack->len; ++i) {
            ...
            if(playlist != NULL) {
                result =
                    g_list_prepend(result, g_strdup(mpd_playlist_get_path(playlist)));
            }
        }
    }
    moose_stprv_unlock(self->priv);
...
static void moose_store_init(MooseStore *self) {
    ...
    g_signal_connect(priv->jm, "dispatch", G_CALLBACK(moose_store_job_execute_callback),
                     self);
}
...
static void moose_store_finalize(GObject *gobject) {
    ...
    g_signal_handlers_disconnect_by_func(self->priv->client, moose_store_update_callback,
                                         self);
    g_signal_handlers_disconnect_by_func(self->priv->client,
                                         moose_store_connectivity_callback, self);
...
static void moose_store_get_property(GObject *object,
                                     guint property_id,
                                     GValue *value,
                                     GParamSpec *pspec) {
...
static void moose_store_set_property(GObject *object,
                                     guint property_id,
                                     const GValue *value,
                                     GParamSpec *pspec) {
    ...
    case PROP_TOKENIZER:
        if(g_value_get_string(value)) {
            strncpy(priv->settings.tokenizer,
...
static void moose_store_class_init(MooseStoreClass *klass) {
    ...
    pspec = g_param_spec_object("full-playlist",
                                "Full playlist",
    ...
    pspec = g_param_spec_string("db-directory",
                                "Database Directory",
    ...
    pspec = g_param_spec_boolean("use-compression",
                                 "Use compression",
                                 "Compress the backup on the filesystem?",
                                 TRUE, /* default value */
                                 G_PARAM_READWRITE);
    ...
    pspec = g_param_spec_boolean("use-memory-db",
                                 "Use Memory DB",
    ...
    pspec = g_param_spec_string("tokenizer",
                                "Tokenizer",
...
MooseStore *moose_store_new(MooseClient *client) {
    return g_object_new(MOOSE_TYPE_STORE, "client", client, NULL);
}
...
MooseStore *moose_store_new_full(MooseClient *client,
    ...
    return g_object_new(MOOSE_TYPE_STORE, "client", client, "db-directory", db_directory,
                        "tokenizer", tokenizer, "use-memory-db", use_memory_db,
                        "use-compression", use_compression, NULL);
}

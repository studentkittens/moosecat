...
GRegex *REGEX_QUOTES = NULL, *REGEX_RANGES = NULL;
...
const char *moose_store_qp_tag_abbrev_to_full(const char *token, size_t len) {
    static const char *abbrev_list[] = {['a'] = "artist:",       ['b'] = "album:",
                                        ['c'] = "album_artist:", ['d'] = "duration:",
                                        ['g'] = "genre:",        ['n'] = "name:",
                                        ['p'] = "performer:",    ['r'] = "track:",
                                        ['s'] = "disc:",         ['y'] = "date:",
                                        ['t'] = "title:",        ['u'] = "uri:",
...
static void moose_store_qp_write_string(MooseStoreParseData *data, const char *string,
                                        size_t len) {
...
void moose_store_qp_process_single_word(MooseStoreParseData *data, const char *text,
                                        size_t len) {
    if(len < 1) {
        return;
    }
...
void moose_store_qp_process_text(MooseStoreParseData *data, const char *text,
                                 size_t len) {
    bool is_tag = false;
    ...
    if((tag_middle = moose_store_qp_is_tag_text(text, len)) != NULL) {
        ...
        if(full == NULL) {
            if(moose_store_qp_is_valid_tag(text, tag_middle - text) == false) {
        ...
        /* Skip : */
        moose_store_qp_write_string(data, tag_middle, rest_len);
        g_string_append_unichar(data->output, ' ');
    } else if(data->check.was_tag) {
        moose_store_qp_write_string(data, text, len);
    } else {
        moose_store_qp_process_single_word(data, text, len);
        g_string_append_unichar(data->output, ' ');
    }
...
bool moose_store_qp_check_is_near_token(char *iter) {
    ...
    default:
        return moose_store_qp_check_is_soft_token(iter);
    }
}

gunichar moose_store_qp_get_prev_char(MooseStoreParseData *data,
...
void moose_store_qp_process_operand(MooseStoreParseData *data) {
    ...
    if(data->previous_char == '\\') {
        ...
        g_string_append(data->output,
                        moose_store_qp_special_char_to_full(data->current_char));
        data->check.was_tag = false;
    }
}
...
void moose_store_qp_process_token(MooseStoreParseData *data) {
    ...
    for(;;) {
        ...
        if(end_token != NULL) {
            if(moose_store_qp_get_prev_char(data, end_token) == '\\') {
                end_token++;
                continue;
            } else {
    ...
    if(strncmp(data->iter, "NOT", 3) == 0 || strncmp(data->iter, "AND", 3) == 0 ||
       strncmp(data->iter, "OR", 2) == 0) {
         ...
         * valid to (AND OR NOT) */
        data->check.bad_conjunction = false;
        moose_store_qp_process_text(data, data->iter, token_len);
    }
...
static gboolean moose_store_qp_quote_eval_cb(const GMatchInfo *info, GString *res,
                                             gpointer data) {
...
static char *moose_store_qp_preprocess_quotationmarks(const char *query,
                                                      MooseStoreParseData *data) {
    if(query == NULL) {
        return NULL;
    }
    ...
    if(REGEX_QUOTES == NULL) {
        REGEX_QUOTES =
            g_regex_new("(\\w:\\s*|)\"(\\s*\\w:\\s*|)(.*?)\"", G_REGEX_OPTIMIZE, 0, NULL);

        atexit(moose_mtx_free_quotes);
    }

    char *result = g_regex_replace_eval(REGEX_QUOTES, query, -1, 0, 0,
                                        moose_store_qp_quote_eval_cb, data, NULL);
...
static gboolean moose_store_qp_range_eval_cb(const GMatchInfo *info, GString *res,
                                             gpointer data) {
    MooseStoreParseData *parse_data = data;
    char *tag = g_match_info_fetch(info, 1);
 ...
 * */
static char *moose_store_qp_preprocess_ranges(const char *query,
                                              MooseStoreParseData *data) {
    if(query == NULL) {
        return NULL;
    }
    ...
    if(REGEX_RANGES == NULL) {
        REGEX_RANGES =
            g_regex_new("(\\w:|)(\\d*)(\\.{2,3})(\\d+)", G_REGEX_OPTIMIZE, 0, NULL);

        atexit(moose_mtx_free_ranges);
    }

    char *result = g_regex_replace_eval(REGEX_RANGES, query, -1, 0, 0,
                                        moose_store_qp_range_eval_cb, data, NULL);
    /* } */
    G_UNLOCK(REGEX_RANGES);
...
char *moose_store_qp_parse(const char *query, const char **warning, int *warning_pos) {
    ...
    data->query = moose_store_qp_preprocess(query, data);
    ...
    while(*(data->iter)) {
        ...
        data->previous_char = moose_store_qp_get_prev_char(data, data->iter);

        if(!moose_store_qp_check_is_soft_token(data->iter)) {
            switch(data->current_char) {
            case '*':
                moose_store_qp_process_star(data);
                break;
            case '+':
            case '|':
            case '!':
            ...
            moose_store_qp_process_empty(data);
        }

...
enum { PROP_STORE = 1, PROP_N };

G_DEFINE_TYPE_WITH_PRIVATE(MooseStoreCompletion, moose_store_completion, G_TYPE_OBJECT);
...
static void moose_store_completion_client_event(G_GNUC_UNUSED MooseClient *client,
...
static void moose_store_completion_get_property(GObject *object,
                                                guint property_id,
                                                GValue *value,
                                                GParamSpec *pspec) {
...
static void moose_store_completion_set_property(GObject *object,
                                                guint property_id,
                                                const GValue *value,
                                                GParamSpec *pspec) {
    MooseStoreCompletionPrivate *priv = MOOSE_STORE_COMPLETION(object)->priv;
...
static char *moose_store_completion_normalize_string(const char *input) {
    ...
    if(utf8_lower != NULL) {
        char *normalized =
            g_strstrip(g_utf8_normalize(utf8_lower, -1, G_NORMALIZE_DEFAULT));
...
static art_tree *moose_store_completion_create_index(MooseStoreCompletion *self,
                                                     MooseTagType tag) {
    g_assert(self);
    ...
    moose_store_gw(self->priv->store,
                   moose_store_query(self->priv->store, "*", FALSE, copy, -1));
    ...
    for(size_t i = 0; i < copylen; ++i) {
        ...
        if(song != NULL) {
            ...
            if(word != NULL) {
                ...
                art_insert(tree, (unsigned char *)normalized, strlen(normalized),
                           GINT_TO_POINTER(i));
                g_free(normalized);
            }
        }
...
static int moose_store_completion_art_callback(void *user_data, const unsigned char *key,
...
char *moose_store_completion_lookup(MooseStoreCompletion *self, MooseTagType tag,
                                    const char *key) {
    g_assert(self);
    g_assert(tag < MOOSE_TAG_COUNT);
    ...
    art_iter_prefix(tree, (unsigned char *)normalized_key, strlen(key),

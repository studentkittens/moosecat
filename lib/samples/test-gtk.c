...
enum { COLUMN_ARTIST = 0, COLUMN_ALBUM, COLUMN_TITLE, NUM_COLUMNS };
...
static GtkTreeModel *create_model(void) {
    ...
    return GTK_TREE_MODEL(
        gtk_list_store_new(NUM_COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING));
}
...
static void update_view(EntryTag *tag, const char *search_text) {
    ...
    g_object_ref(
        model); /* Make sure the model stays with us after the tree view unrefs it */
    ...
    for(int i = 0; i < found; i++) {
        ...
        gtk_list_store_set(list_store, &iter, COLUMN_ARTIST,
                           moose_song_get_tag(song, MOOSE_TAG_ARTIST), COLUMN_ALBUM,
                           moose_song_get_tag(song, MOOSE_TAG_ALBUM), COLUMN_TITLE,
                           moose_song_get_tag(song, MOOSE_TAG_TITLE), -1);
    }

    gtk_tree_view_set_model(GTK_TREE_VIEW(tag->view),
                            model); /* Re-attach model to view */
...
static EntryTag *setup_client(void) {
    ...
    moose_client_connect(client, "localhost", 6600, 2.0);

    if(client && moose_client_is_connected(client)) {
        client_setup = g_timer_elapsed(setup_timer, NULL);
        g_timer_start(setup_timer);
        MooseStore *store = moose_store_new_full(client, NULL, NULL, false, false);
...
static void build_gui(EntryTag *tag) {
    ...
    tag->view = treeview;
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scw), GTK_POLICY_AUTOMATIC,
                                   GTK_POLICY_ALWAYS);
    gtk_tree_view_set_fixed_height_mode(treeview, true);
    static const char *column_desc[] = {"Artist", "Album", "Title"};
    ...
    for(gsize i = 0; i < sizeof(column_desc) / sizeof(char *); i++) {
        ...
        column = gtk_tree_view_column_new_with_attributes(column_desc[i], renderer,
                                                          "text", i, NULL);

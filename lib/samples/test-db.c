            ...
            for(;;) {
                ...
                if(strncmp(line_buf, ":spl ", 5) == 0) {
                    ...
                    if(args != NULL) {
                        ...
                        moose_store_gw(
                            db, moose_store_playlist_query(db, stack, args[1], args[2]));
                        g_print("%s %s\n", args[1], args[2]);
                ...
                if(strncmp(line_buf, ":ls ", 4) == 0) {
                    ...
                    if(args != NULL) {
                        ...
                        moose_store_gw(
                            db, moose_store_query_directories(db, stack, query, depth));
                ...
                moose_store_gw(db, moose_store_query(db, line_buf, queue_only, song_buf,
                                                     song_buf_size));

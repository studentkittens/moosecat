...
const MooseIdle ON_STATUS_UPDATE_FLAGS =
    0
    | MOOSE_IDLE_PLAYER
    | MOOSE_IDLE_OPTIONS
    | MOOSE_IDLE_MIXER
...
const MooseIdle ON_STATS_UPDATE_FLAGS =
    ...
    | MOOSE_IDLE_DATABASE
    ;
const MooseIdle ON_CURRENT_SONG_UPDATE_FLAGS =
    0
    | MOOSE_IDLE_PLAYER
    ;
const MooseIdle ON_REPLAYGAIN_UPDATE_FLAGS =
    0
    | MOOSE_IDLE_OPTIONS
    ;
...
static void moose_report_connectivity(
    MooseClient * self,
    const char * new_host,
    int new_port,
    float new_timeout) {
...
static gboolean moose_client_check_error_impl(MooseClient * self, struct mpd_connection * cconn, gboolean handle_fatal) {
    ...
    if (error != MPD_ERROR_SUCCESS) {
        ...
        moose_critical("Error #%d: %s (fatal=%s)",
                       error, error_message, is_fatal ? "yes" : "no"
                      );
...
gboolean moose_client_check_error_without_handling(MooseClient * self, struct mpd_connection * cconn) {
...
static void moose_updata_data_push(MooseClient * self, MooseIdle event, gboolean is_status_timer) {
    g_assert(self);
...
long moose_client_send_single(MooseClient * self, const char * command_name) {
    ...
    if(handler != NULL) {
        if(strcmp(handler->format, "(s)") != 0) {
            moose_warning(
                "moose_client_send_single(\"%s\") requires arguments.", command_name
            );
            return -1;
        }

        return moose_job_manager_send(
                   self->priv->jm, 0, g_variant_new("(s)", command_name)
               );
    } else {
        return -1;
    }
...
static gpointer moose_update_thread(gpointer user_data) {
    ...
    while ((event_mask = GPOINTER_TO_INT(g_async_queue_pop(self->priv->event_queue))) != MOOSE_THREAD_TERMINATOR) {

...
enum { SIGNAL_THREAD, SIGNAL_DELIVER, N_SIGNALS };

enum { PROP_MAX_THREADS = 1, N_PROPS };
...
G_DEFINE_TYPE_WITH_PRIVATE(MooseThreads, moose_threads, G_TYPE_OBJECT);
...
static void moose_threads_get_property(GObject *object,
                                       guint property_id,
                                       GValue *value,
                                       GParamSpec *pspec) {
...
static void moose_threads_set_property(GObject *object,
                                       guint property_id,
                                       const GValue *value,
                                       GParamSpec *pspec) {
    MooseThreads *self = MOOSE_THREADS(object);
...
static void moose_threads_class_init(MooseThreadsClass *klass) {
    ...
    g_object_class_install_property(
        ...
        g_param_spec_int("max-threads", "Max. Threads",
                         "Maximum number of threads to launch", 0, G_MAXINT, 10,
     ...
     */
    SIGNALS[SIGNAL_THREAD] =
        g_signal_new("thread", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                     NULL, G_TYPE_POINTER /* return_type */, 1 /* n_params */,
     ...
     */
    SIGNALS[SIGNAL_DELIVER] = g_signal_new(
        "deliver", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL,
        G_TYPE_NONE /* return_type */, 1 /* n_params */, G_TYPE_POINTER /* param_types */
        );
}
...
static void moose_threads_init(MooseThreads *self) {
    ...
    self->priv->pool = g_thread_pool_new(moose_threads_dispatch, self, 10, FALSE, &error);

...
G_DEFINE_TYPE_WITH_PRIVATE(MooseZeroconfBrowser, moose_zeroconf_browser, G_TYPE_OBJECT);
G_DEFINE_TYPE_WITH_PRIVATE(MooseZeroconfServer, moose_zeroconf_server, G_TYPE_OBJECT);

enum { BROWSER_SIGNAL_STATE_CHANGED, BROWSER_NUM_SIGNALS };

static guint BROWSER_SIGNALS[BROWSER_NUM_SIGNALS];

static void moose_zeroconf_call_user_callback(MooseZeroconfBrowser* self,
                                              MooseZeroconfState state) {
    g_assert(self);
...
static void moose_zeroconf_drop_server(MooseZeroconfBrowser* self,
                                       const char* name,
                                       const char* proto,
                                       const char* domain) {
    for(GList* iter = self->priv->server_list; iter; iter = iter->next) {
        ...
        if(server != NULL) {
            if(1 && g_strcmp0(server->priv->name, name) == 0 &&
               g_strcmp0(server->priv->protocol, proto) == 0 &&
               g_strcmp0(server->priv->domain, domain) == 0) {
                self->priv->server_list =
                    g_list_delete_link(self->priv->server_list, iter);
...
static void moose_zeroconf_check_client_error(MooseZeroconfBrowser* self,
                                              const gchar* prefix_message) {
    /* Print just a message for now */
    if(avahi_client_errno(self->priv->client) != AVAHI_OK) {
        g_free(self->priv->last_error);
        self->priv->last_error =
            g_strdup_printf("%s: %s", prefix_message,
...
static void moose_zeroconf_resolve_callback(AvahiServiceResolver* resolver,
    ...
    case AVAHI_RESOLVER_FAILURE: {
        gchar* format =
            g_strdup_printf("Failed to resolve service '%s' of type '%s' in domain '%s'",
                            name, type, domain);
    ...
    case AVAHI_RESOLVER_FOUND: {
        ...
        g_object_set(server, "name", name, "protocol", type, "domain", domain, "host",
                     host_name, "addr", addr_buf, "port", port, NULL);
...
static void moose_zeroconf_service_browser_callback(
    ...
    case AVAHI_BROWSER_NEW: {
        if(avahi_service_resolver_new(full_client, interface, protocol, name, type,
                                      domain, (AvahiLookupFlags)AVAHI_PROTO_INET,
                                      (AvahiLookupFlags)0,
                                      moose_zeroconf_resolve_callback, self) == NULL) {
    ...
    /* One-time event, to notify the user that more records will probably not show up in
     * the near future, i.e.
    ...
    /* Browsing failed due to some reason which can be retrieved using
...
static void moose_zeroconf_register_browser(MooseZeroconfBrowser* self) {
    ...
    avahi_service_browser_new(self->priv->client,
                              AVAHI_IF_UNSPEC,
                              AVAHI_PROTO_UNSPEC,
...
static void moose_zeroconf_client_callback(AVAHI_GCC_UNUSED AvahiClient* client,
...
static void moose_zeroconf_browser_class_init(MooseZeroconfBrowserClass* klass) {
     ...
     */
    BROWSER_SIGNALS[BROWSER_SIGNAL_STATE_CHANGED] =
...
static void moose_zeroconf_server_get_property(GObject* object,
                                               guint property_id,
                                               GValue* value,
                                               GParamSpec* pspec) {
...
static void moose_zeroconf_server_set_property(GObject* object,
                                               guint property_id,
                                               const GValue* value,
                                               GParamSpec* pspec) {
    MooseZeroconfServer* self = MOOSE_ZEROCONF_SERVER(object);
...
static void moose_zeroconf_server_class_init(MooseZeroconfServerClass* klass) {
    ...
    pspec = g_param_spec_string("domain",
                                "Serverdomain",
    ...
    pspec = g_param_spec_string("addr",
                                "Serveraddress",
...
GType moose_zeroconf_state_get_type(void) {
    ...
    if(enum_type == 0) {
        static GEnumValue state_types[] = {
            {MOOSE_ZEROCONF_STATE_UNCONNECTED, "MOOSE_ZEROCONF_STATE_UNCONNECTED",
             "unconnected"},
            {MOOSE_ZEROCONF_STATE_CONNECTED, "MOOSE_ZEROCONF_STATE_CONNECTED",
             "connected"},
            {MOOSE_ZEROCONF_STATE_ERROR, "MOOSE_ZEROCONF_STATE_ERROR", "error"},
            {MOOSE_ZEROCONF_STATE_CHANGED, "MOOSE_ZEROCONF_STATE_CHANGED", "changed"},
            {MOOSE_ZEROCONF_STATE_ALL_FOR_NOW, "MOOSE_ZEROCONF_STATE_ALL_FOR_NOW",
             "all-for-now"},
            {0, NULL, NULL}};

        enum_type = g_enum_register_static("MooseZeroconfState", state_types);
    }
